...
public class IBTExam {
	...
	@Given("the candidate taking credentials from the attendance sheet")
	public void the_candidate_taking_credentials_from_the_attendance_sheet() throws InterruptedException {
		AttdReport=new AttendanceReport(BaseClass.getDriver());  //IMPORTANT 
		AttdReport.gotoReport();
		Thread.sleep(3000);
		AttdReport.gotoAttendReport();
		AttdReport.clickExamName();
	    AttdReport.selectExamName("IBT exam");
		AttdReport.clickReportBy();
		Thread.sleep(3000);
		AttdReport.selectReportBy("Exam");
	...
	/*	if(Caddport.verifyDashboard()==true)
		{
			System.out.println("Exam name is IBT");
		}
		else {System.out.println("Exam Name is missing");}  */
	...
	{
		Caddport=new CandidatePortal(BaseClass.getDriver());
		boolean exs=Caddport.verifyExamStatus();
